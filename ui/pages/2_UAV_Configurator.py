"""
ui/pages/2_UAV_Configurator.py
...
st.set_page_config(page_title="UAV Configurator", page_icon="🛠️", layout="wide")
...
st.page_link("pages/8_User_Guide.py", label="📖 User Guide — UAV Configurator", icon=None)
...
with tab_equip:
    ...
    if db.equipment:
        ...
        for eq in db.equipment.values():
            equip_rows.append({
                ...
                "Operating W":  eq.operating_power_w,
    ...
    with st.expander("➕ Add New Equipment Item", expanded=False):
        ...
        with col_b:
            ...
            new_idle_w      = st.number_input("Idle power (W)", 0.0, 5000.0, 0.0, 1.0, key="eq_idle_w",
                                               help="Draw when powered on but not working")
            new_operating_w = st.number_input("Operating power (W)*", 0.0, 50000.0, 0.0, 10.0, key="eq_op_w",
                                               help="Normal working draw")
        with col_c:
            new_max_w      = st.number_input("Max power (W)", 0.0, 100000.0, 0.0, 10.0, key="eq_max_w",
                                              help="Peak draw (e.g. servo slewing, burst)")
        ...
        if st.button("💾 Save Equipment to Database", type="primary", key="eq_save"):
            if not new_equip_id:
                ...
                try:
                    from batteries.models import Equipment as _Eq
                    new_eq = _Eq(
                        equip_id=new_equip_id,
                        category=new_category,
                        manufacturer=new_mfr,
                        model=new_model,
                        nom_voltage_v=new_nom_v,
                        nom_current_a=new_nom_a,
                        idle_power_w=new_idle_w,
                        operating_power_w=new_operating_w,
                        max_power_w=new_max_w,
                        weight_g=new_weight_g,
                        efficiency_pct=new_eff_pct,
                        duty_cycle_pct=new_duty_pct,
                        notes=new_notes,
                        active=new_active,
                    )
                    db.equipment[new_equip_id] = new_eq
                    db.save_equipment()
                    reload_db()
    ...
    with st.expander("✏️ Edit Equipment Power Values", expanded=False):
        st.caption("Update power levels for an existing equipment item.")
        ...
        if not all_eq_ids:
            ...
            with col_e1:
                edit_idle_w      = st.number_input("Idle W",      0.0, 50000.0,  float(eq_obj.idle_power_w),      1.0, key="eq_edit_idle",
                                                   help="Draw when powered on but not working")
            with col_e2:
                edit_operating_w = st.number_input("Operating W", 0.0, 50000.0,  float(eq_obj.operating_power_w), 1.0, key="eq_edit_op",
                                                   help="Normal working draw")
            with col_e3:
            ...
            with col_e5:
                edit_active = st.checkbox("Active", value=eq_obj.active, key="eq_edit_active")
            ...
            if st.button("💾 Update Equipment", key="eq_edit_save"):
                try:
                    ...
                    updated = dataclasses.replace(
                        eq_obj,
                        idle_power_w=edit_idle_w,
                        operating_power_w=edit_operating_w,
                        max_power_w=edit_max_w,
                        weight_g=edit_weight,
                        active=edit_active,
                    )
                    db.equipment[edit_eq_id] = updated
                    db.save_equipment()
                    reload_db()
    ...
    if db.uav_configs:
        ...
        for uid, uav in db.uav_configs.items():
            uav_summary.append({
                ...
                "Operating W (est.)": round(uav.phase_power_w(), 0),
            })
        ...
        if uav_ids:
            ...
            for eq, qty in view_uav.equipment_list:
                view_rows.append({
                    ...
                    "Model":               eq.model,
                    "Qty":                 qty,
                    "Idle W (total)":      round(eq.idle_power_w      * qty, 1),
                    "Operating W (total)": round(eq.operating_power_w * qty, 1),
            ...
            c2.metric("Operating power", f"{view_uav.phase_power_w():.0f} W")
            ...
            if st.session_state["new_uav_items"]:
                ...
                for eid, qty in st.session_state["new_uav_items"]:
                    ...
                    if eq:
                        ...
                        total_op_w += eq.operating_power_w * qty
                st.dataframe(pd.DataFrame(preview_rows), use_container_width=True, hide_index=True)
                c1, c2 = st.columns(2)
                c1.metric("Total weight", f"{total_w:.0f} g")
                c2.metric("Total operating W", f"{total_op_w:.0f} W")
